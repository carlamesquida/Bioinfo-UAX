...
hist(data)
hist(data, col="gray", main="GSE5583 - Histogram") *para cambiar de color y nombre

# Transformamos los datos con un logaritmo 
# ¿Qué pasa si hacemos una transformación logarítima de los datos? ¿Para qué sirve? Usamos log para que los datos del hist se vean mejor
data_log=log2(data) 
hist(data_log)*histograma en campana de gauss

# Hacemos un boxplot con los datos transformados. ¿Qué significan los parámetros que hemos empleado?
# ¿Qué es un boxplot? Es un tipo de gráfica que muestra un resumen de una gran cantidad de datos (mediana, moda, cuartiles), además de intuir su morfología y simetría.
...
*las=2 es para poner los nombres en vertical

# Hacemos un hierarchical clustering de las muestras basándonos en un coeficiente de correlación
# de los valores de expresión. ¿Es correcta la separación? Sí es correcta, pues tenemos los WT agrupados y por otra parte los KO.
...
wt <- data[,1:3] *corchete para acceder a datos y coma para separar las columnas 
ko <- data[,4:6]
class(wt)*class dice el tipo de dato que tenemos
head(wt) *head para ver la estructura y nos da las primeras líneas (encabezados)

# Calcula las medias de las muestras para cada condición. Usa apply
wt.mean = apply(wt, 1, mean)*apply donde yo quiera que haga la media
head(wt.mean)
ko.mean = apply(ko, 1, mean)
head(ko.mean)

# ¿Cuál es la media más alta? La de KO
max(wt.mean) 
max(ko.mean)
...
abline(0, 1, col = "red") *con el plot abierto
abline(h=2, col = "blue") *linea eje x
abline(v=5, col = "green") *linea eje y
...
hist(diff.mean)
hist(diff.mean, col = "pink") *cambiar el color del hist

# Calculamos la significancia estadística con un t-test. p> 0,5 = insignificativo
...
# ¿Cuántas valores tiene cada condición? Cada gen tiene 6 valores, que son 6 muestras (3 para cada condición) y 2 condiciones (wt, ko).

pvalue = NULL
tstat = NULL
for(i in 1 : nrow(data)) { #Para cada gen *nrow = número de filas total, 
	x = wt[i,] # gene wt número i 
	y = ko[i,] # gene ko número i

	# Hacemos el test
	t = t.test(x, y) *contra los valores de la otra condición

	#Añadimos el p-value a la lista
	pvalue[i] = t$p.value
	#Añadimos las estadísticas de la lista
	tstat[i] = t$statistic
}
head(pvalue)
length(pvalue)
...
# ¿Qué pasa si le ponemos con una transformación de -log10? Para cambiar la escala y reflajar mas la proporción de la distribución
hist(-log10(pvalue), col = "green")

# Hacemos un volcano plot. Aquí podemos meter la diferencia de medias y la significancia (*está arriba) estadística
...
#abline((v = diff.mean_cutoff, col = "red", lwd = 3) *se solapan
...
#Colv y Row son los dendogramas
